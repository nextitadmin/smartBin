...
import {
  PayBillDto,
  BillResponseDto,
  FacilityBillResponseDto,
  CorporateBillResponseDto,
} from './dtos/bill.dto';
...
export class BillService {
  ...
  async payBill(
    ...
    if (dto.paymentMethod === PaymentMethod.Wallet) {
      ...
      return new SuccessResponse(
        'Bill paid successfully via wallet',
        new BillResponseDto(),
      );
    }
  ...
  async payFacilityBill(
    userId: string,
    dto: PayBillDto,
    user: any,
  ): Promise<SuccessResponse<FacilityBillResponseDto | any>> {
    const bill = await this.billModel.findOne({
      userId,
      billId: dto.billId,
    });
    ...
    if (dto.paymentMethod === PaymentMethod.Wallet) {
      ...
      return new SuccessResponse(
        'Facility Manager bill paid successfully',
        response,
      );
    }
  ...
  async payCorporateBill(
    ...
    dto: PayBillDto,
    user: any,
  ): Promise<SuccessResponse<CorporateBillResponseDto | any>> {
    const bill = await this.billModel.findOne({
      userId,
      billId: dto.billId,
    });
