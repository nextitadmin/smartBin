...
export class ReportService {
  ...
  // payemnt summary
  private async getPaymentSummary(user: AuthUser, page: number, limit: number) {
    const offset = (page - 1) * limit;

    console.log('Fetching payment history for user:', user);
    const filter = {
      userId: user.id, // no need for new Types.ObjectId here // already hanled in the pre hook
    };

    const tx = await this.transactionModel.find(filter);
    console.log('Transactions found:', tx.length, filter);
    return tx;
